...
За год можно ожидать прирост пользователей до `13 %` => `249 TB * 0.13 = 32 TB 40 GB` нового пространстра может
потребоваться.
...
размещать стоит ближе всеко к аудитории приложения.
...
1. Таблица `Preview` полностью будет в оперативной памяти. Но всю её не получится поместить в память одной машины,
...
1. На хранение всех фото нужно до `250 TB`, использование S3 может быть слишком дорогим в долгосрочной перспективе.
...
- Резервирование PostgreSQL — [схема реплецирования](#Репликация)
...
- **Tarantool:**
    - Два шарда, каждый должен хранить ~1.2 TB в памяти. В среднем 7000 RPS чтение на шард. Трафик ~55 Mbit/s
- **ClickHouse:**
    - 15 Tb данных, 1260 RPS запись, Трафик < 1 Mbit/s
- **Elasticsearch - поиск:**
    - Объем данных незначительный, В среднем 140 RPS
- **CEPH:**
    - Объем данных ~249 TB, 1120*2 RPS , Трафик ~2.2 Gbit/s
- **PostgreSQL:**
    - Объем данных ~1 TB, чтение 1680 RPS, Запись 40 RPS Трафик < 20 Mbit/s
- **LanceDB**:
    - Объем данных ~100 GB, чтение 200 RPS, Запись 100 RPS Трафик < 2 Mbit/s

### Балансировка

**Nginx**: до 16000 RPS, также используется в качетве балансировщиков перед каждым из сервисов
...
|------------------------|-------------|----------|--------|-------|
| Tarantool              | 16          | 1536     | 32 GB  | 2     |
| ClickHouse             | 16          | 32       | 24 TB  | 1     |
| Elasticsearch - поиск  | 8           | 32       | 32 GB  | 1     |
| CEPH                   | 64          | 256      | 250 TB | 1     |
| PostgreSQL             | 16          | 256      | 2 TB   | 1     |
| CRUD Service           | 8           | 16       | 8 GB   | 2     |
| Images Service         | 4           | 8        | 8 GB   | 2     |
| Recommendation Service | 8           | 16       | 8 GB   | 2     |
| API Gateway            | 8           | 16       | 8 GB   | 2     |
| Load Balancer          | 6           | 32       | 8 GB   | 6     |
...
|-----------------------|------------------------------------------|-------|
| Tarantool             | 2x6338 (32) / 16x96GB / 1x NVMe 32 GB    | 2     |
...
| Serer for kuber       | 2x6338 (64) / 4x4GB / 1x NVMe 8 GB       | 2     |
