...
Больше 80% всех объявлений находятся в Заподной или Средней частях России. Поэтому ДЦ лучше всего разместить в Москве.
Но чтобы обеспечить быстроту ответа для пользователей с востока - стоит также использовать ДЦ в Иркутске.
...
|---------|------------------------|---------------------------------|----------------|
| Москва  | Западная часть России  | 80                              | 12000          |
| Иркутск | Восточная часть России | 20                              | 3000           |
...
Будет использоваться **физическая репликация** всей БД в СУБД PostgreSQL.
...
Так как таблицы в PostgresSQL не очень большие, то все индексы можно поместить в память одной машины. Поэтому шардинг
для PostgresSQL можно не использовать.
