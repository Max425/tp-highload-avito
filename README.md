...
Для Мультиплексирование подключений будем использовать **PgBouncer**[^7].

#### Резервирование

Для резервирования будем делать **Снапшоты файловой системы**[^8].
...
2. GiST: Distance_search.point, Name_search.name
...
3. User: hash(id) mod 1000
4. User_rating: hash(user_id) mod 1000
...
**Clickhouse**, куда будут писаться нужные данные для аналитики.
