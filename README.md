...
| kubenode              | own     | 2x6338 / 1x128MB / 1x NVMe 256 MB                            | 8     | 28    | -               | -                  |

Для всех сервисов нужно 14 kubenode, но у каждого сервиса есть реплика, поэтому берем с запасом x2.

Все kubenode будут разнесены на 4 физических сервера. На каждом сервере будет 7 kubenode (для обеспечения
отказоусточивости).

| Сервис | Хостинг | Configuration                        | Cores | Count | Покупка[^22], $ | Аренда[^23], $/мес |
|--------|---------|--------------------------------------|-------|-------|-----------------|--------------------|
| kuber  | own     | AMD EPYC 9634 / 1x4GB / 1x NVMe 8 GB | 32    | 4     | 6000            | 550                |
...
- Стоимость одного датацентра:  `2*5000 + 2*2000 + 2*8000 + 2*5500 + 2*2000 + 2*1000 + 2*5000 + 4*6000 = 81000$`
- Будем предполагать обновление железа 1 раз в 5 лет, тогда стоимость в год будет: `81000 / 5 = 16200$`
- Поскольку у нас 2 датацентра, то общая стоимость железа в год будет `2 * 16200 = 32400$`
